...
| [[lopopolo-openai-extreme-harness-engineering]] | Ryan Lopopolo (OpenAI) | Software Factories / Coding Agents / Code Legibility | May 2026 Latent Space podcast on "harness engineering" article. **Three engineers, 5 months, ~1M LOC, ~1,500 PRs, 0% human-authored code, post-merge review only.** Defining 2026 articulation of harness engineering as a discipline. Introduces [[symphony]] (six-layer Elixir orchestration substrate distributed as a "ghost library"), the **on-policy harness** principle (build native to model output, not as scaffold around it), and harness-engineering-as-text-injection-discipline. Sub-1-minute build invariant; 6 skills total; `dollar-land` autonomous-merge skill; daily-cron agent loops over the team's session logs. Anti-MCP, pro-CLI (`gh` exemplar). In-housing dependencies as the new norm. **Strongest School-1 production datapoint in the wiki — direct counter-evidence to Dex's reversal.** Author-Codex ↔ reviewer-Codex negotiation with P0/P1/P2 framework. "I open Linear twice a day." |
...
| [[openai-codex]] | Tool/Product (model + CLI + App + harness) | OpenAI's coding-agent product line: Codex models within GPT-5 / 5.x, Codex CLI, Codex App (2M WAU, 25% WoW growth as of May 2026), VS Code extension, Slack ChatGPT app. Same harness across surfaces. **Background shells (5.3) and computer use (5.4) are the harness-defining feature additions of early 2026.** Skill-files-and-AGENTS.md primitive system. Exemplary CLI-shaped tool environment (`gh` etc). Reference platform for [[lopopolo-openai-extreme-harness-engineering|Lopopolo's harness-engineering article]]. Counterpart to [[claude-agent-sdk|Claude Code]]; both ship "thin harness, smart model" + cloud-agents + computer use. |
| [[symphony]] | Orchestration substrate (Elixir / BEAM) | Six-layer (policy / config / coordination / execution / integration / observability) plus zero-layer self-modification. Built by [[lopopolo-openai-extreme-harness-engineering|OpenAI Frontier]] team and distributed as a **"ghost library"** — a spec sufficient for a coding agent to reassemble locally. Per-task gen-server daemons, **rework state** that trashes the entire worktree+PR rather than incremental fix, latency-insensitive human review (*"I open Linear twice a day"*). Sister to [[attractor]] (StrongDM, DOT graph) and [[agent-control-plane]] (HumanLayer, K8s) at the orchestration tier. Spec was itself built by Ralph-looping Codex against the proprietary upstream repo — Ralph at meta-level. |
...
| [[research-report/ai-in-sdlc-research-report]] | Industry | Polished ~7-page formal-academic MSIS research report. Same five-analysis synthesis with strict adherence to the assignment template: abstract, field overview, six-block techniques section (LLMs+tool use, agentic loops, multi-agent orchestration, SDD, persistent memory, harness), single applied example (Amazon Kiro), seven categories of risks/ethics, six-prediction 2027–2031 outlook, six MSIS-practice takeaways. Restates the four-property predicate as a portfolio-screening tool. |
